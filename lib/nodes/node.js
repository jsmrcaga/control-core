...
class Node extends StateMachine {
	...
	// Actual execution
	execute({ inputs, outputs, context, config, parent_id } = {}) {
		this.to(STATES.EXECUTING);
		return this.#promisify(() => (
			this.run({ inputs, outputs, context, config, parent_id })
		));
	}

	run({ inputs, outputs, context, config, parent_id } = {}) {
	...
	post_run({ inputs, outputs, context, config } = {}) {
		return true;
	}
